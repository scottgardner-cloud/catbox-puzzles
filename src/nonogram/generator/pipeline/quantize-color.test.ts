...
describe('quantizeColor', () => {
  ...
  it('sorts palette deterministically by luminance', () => {
    ...
    const data = new Uint8ClampedArray([
      ...
      255,
      0,
      255, // yellow (high luminance)
      0,
      0,
      255,
      255, // blue (low luminance)
    ]);
    const grid = createPixelGrid(2, 1, data);
    const result = quantizeColor(grid, 2, { kind: 'none' });
    ...
    const lum0 =
      0.299 * result.palette[0].r + 0.587 * result.palette[0].g + 0.114 * result.palette[0].b;
    const lum1 =
      0.299 * result.palette[1].r + 0.587 * result.palette[1].g + 0.114 * result.palette[1].b;
